...
# [ MLEFT, CLEFT, BOAT, MRIGHT, CRIGHT, COST, ESTIMATE, FUNCTION, [ ACTION ], [ PARENT ] ]
initialState = [3, 3, 0, 0, 0, 0, 0, 0]
arrfringe = []
closedarr = []
goalState = [0, 0, 1, 3, 3]
TempArr = []
...
def Heuristic(MLeft, CLeft, Boat):
    if( Boat == 0 ):
        if( MLeft+CLeft <= 2 ):
            return 1
        else:
            return ( (MLeft+CLeft - 2 ) * 2 + 1 )
    else:
...
def findnewState(action, state):
    ...
    newState.append(Heuristic( newState[0], newState[1], newState[2] ))
    newState.append(newState[5]+newState[6])
...
def priorityQue():
    arrfringe.sort(key=lambda x:x[7])
